...
class Detectorc(caffe.Net):
    ...
    def detect_windows(self, image, windows):
        """
        ...
        Parameters
        ----------
        images_windows: (image, window list) iterable.
        ...
        """
        windows, windows_detect = tee(windows)
        # Extract windows.
        window_inputs = []
        ...
        print predictions
        # print windows
        # Package predictions with image and windows.
        detections = []
        ix = 0
        for window in windows_detect:
            detections.append({'window': window, 'prediction': predictions[ix]})
            ix += 1
        return detections
